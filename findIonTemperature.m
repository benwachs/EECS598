...
TELOC = size(x, 2) -1 ;   % Index of electron temperature
TGLOC = size(x, 2);
...
for i = 1:numel(posIons)
    v(:, i) = posIons(i).mobility*c.qe*x(:, TELOC)./c.kb./c.lambda;
end

Ti = x(:, TGLOC) + 1./3./c.kb.*sum(moleFractions(:, posIonLocs).*[posIons(:).mass].*v(i).^2, 2);